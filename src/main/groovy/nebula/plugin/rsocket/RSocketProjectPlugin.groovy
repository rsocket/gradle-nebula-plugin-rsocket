...
class RSocketProjectPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        project.plugins.apply RSocketReleasePlugin
        ...
        if (projectType.isLeafProject || projectType.isRootProject) {
            project.plugins.apply(RSocketPublishingPlugin)
        }
        ...
        if (projectType.isLeafProject) {
            ...
            project.plugins.apply(RSocketPerformancePlugin)
            project.plugins.apply(RSocketOsgiPlugin)
            project.plugins.apply(RSocketLicensePlugin)
            ...
            project.tasks.withType(Javadoc) {
                ...
                options {
                    windowTitle = "RSocket Javadoc ${project.version}"

                    if (JavaVersion.current().isJava8Compatible()) {
                        options.addStringOption('Xdoclint:none', '-quiet')
                    }
                }
                options.addStringOption('top').value = '<h2 class="title" style="padding-top:40px">RSocket</h2>'
            }
