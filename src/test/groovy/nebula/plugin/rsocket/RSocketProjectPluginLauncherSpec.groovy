...
class RSocketProjectPluginLauncherSpec extends IntegrationSpec {
    ...
    def setup() {
        ...
        buildFile << """
            ${applyPlugin(RSocketProjectPlugin)}
            apply plugin: 'java'
            license {
                ignoreFailures = true
            }
    ...
    def 'stand it all up'() {
        ...
        new File(projectDir, 'build/docs/javadoc/index.html').text.contains("<title>RSocket Javadoc ${snapshotVer}</title>")
